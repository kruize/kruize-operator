...
COPY vendor/ vendor/
# Avoid go mod download since we have vendored dependencies
# RUN go mod download
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} GODEBUG=asyncpreemptoff=1 go build -mod=vendor -p 1 -a -o manager cmd/main.go
